...
const signin = (username, password) => async (dispatch) => {
    ...
    try {
        const {data} = await Axios.post("https://projectserver10758282.herokuapp.com/auth/login", {username,password});
        dispatch({
...
const register = (name, username, password,email) => async (dispatch) => {
    ...
    try {
        const {data} = await Axios.post("https://projectserver10758282.herokuapp.com/auth/register", {username,password,name,email});
        dispatch({
...
const update = ({ userId, name, email, password }) => async (dispatch, getState) => {
    ...
    try {
      const { data } = await Axios.put("https://projectserver10758282.herokuapp.com/auth/users/" + userId,
        { name, email, password }, {
        headers: {
